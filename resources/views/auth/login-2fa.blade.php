...
<head>
    ...
    <title>2FA Verification - TulipCrypt</title>
    <!-- Google Fonts -->
...
<body class="text-slate-700 antialiased min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md bg-white/70 rounded-3xl p-8 shadow-2xl border border-white/60 relative overflow-hidden flex flex-col items-center">
        ...
        <h2 class="font-extrabold text-2xl text-slate-800 tracking-tight text-center">Verifikasi Kode 2FA</h2>
        <p class="text-xs text-slate-400 text-center mt-2 max-w-sm">Masukkan 6 digit kode keamanan dari aplikasi Google Authenticator Anda untuk masuk ke vault.</p>
        ...
        <!-- Form -->
        <form action="{{ route('login.2fa.submit') }}" method="POST" class="w-full mt-6 space-y-6">
            @csrf

            <!-- 2FA Code Input -->
            <div class="space-y-2">
                <label for="code" class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-wider text-center">KODE VERIFIKASI 2FA</label>
                <input type="text" 
                       name="code" 
                       id="code" 
                       required 
            ...
            <button type="submit" class="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-bold shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-2">
                ...
                <span>Verifikasi & Buka Vault</span>
            </button>
        </form>
